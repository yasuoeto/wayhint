...
どう設定するか。ここは手順だけを書く(なぜこの方法なのかは開発向けの `dev-docs/DECISIONS.md` 0027)。
...
(`dev-docs/DECISIONS.md` 0027 の「やらないこと」)。
...
答えられない(`dev-docs/DECISIONS.md` 0028)。

### WezTerm

ウィンドウごとに app_id を変えられないため、この規約に乗らない。`wezterm cli list-clients` →
`tty_name` → `/proc` の経路で別途対応できることは確認済みだが、未実装(`dev-docs/DECISIONS.md` 0027)。

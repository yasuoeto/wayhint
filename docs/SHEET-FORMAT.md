...
[`SHEETS.md`](SHEETS.md)。ここはシートに書ける項目の全部と、その決まりをまとめる。
...
- コピーされるのは `copy` → `command` → `key` の順に、最初に書いてあるもの。
...
親シートや `include` から混ざったヒントの位置は [`SHEETS.md`](SHEETS.md) §2。
...
yaml-language-server に対応したエディタで key の補完と検証が効く(設定は [`CONFIG.md`](CONFIG.md) の editor)。

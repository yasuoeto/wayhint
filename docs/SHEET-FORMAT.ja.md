...
- app_id・引数・コマンドラインは先頭の 4096 文字にだけ当てる。中身はプログラムが決めるもので、遅い
  pattern でヒント画面を待たせないため。それより長いと `$` は当たらなくなる。
- pattern は [`regex`](https://pypi.org/project/regex/) モジュールで照合する(書き方は Python の `re`
  と同じ)。1 つの文字列に 50 ms を超えてかかる pattern(`^(a|aa)+$` のように際限なくバックトラック
  するもの)は当たらなかったものとして扱い、`wayhintd` がその pattern を名指しで警告ログに出す。
...
| `copy` | | 文字列 | コピーする文字列が表示と違うときだけ書く。違うときは詳細欄に出る。改行などの制御文字は `\n` のように書き出す |
...
- 1 MiB を超えるシートや `config.yaml` は読まず、間違いとして報告する。
...
- シートが symlink(dotfiles のリポジトリへのリンクなど)なら symlink のまま、リンク先を書き換える。

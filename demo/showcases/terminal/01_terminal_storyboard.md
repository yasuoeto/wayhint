...
- 映る sheet(`wm.yaml` / `less.yaml` / `foot.yaml`)は画面で読まれるので、先頭にコメントを置かない。役割はここに書く:
  - `less.yaml` と `vi.yaml` — 端末の中を見る経路の 2 つのプロセス。窓が 2 枚あっても、sheet は前面にいる
    プロセスで決まることを示すための対
  - `wm.yaml` — `match` が無いので窓に対して選ばれることはなく、`config.yaml` の `include` で全 sheet の
    一覧の末尾に混ざる。置くのは窓そのものの操作だけ。overlay の出し方(`Super+H`)は書かない
    (それを忘れている人はこの一覧を開けないので、hint として成立しない)
...
| 43–51 | `Super+H` → sheet が `Vi` に差し替わる | 別のウィンドウで押し直せば、その一番前のプロセスのヒント |
...
| 60–68 | `Super+H` → `Foot › Vi`。Foot の 3 件が vi の後に混ざって 8 件 | 一番前のプロセスのヒントと一緒に出る |

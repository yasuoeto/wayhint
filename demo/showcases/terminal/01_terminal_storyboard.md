# terminal — 端末の中を見る

- **主語**: multiplexer を使っていない端末の窓と、その中で動いているプログラム
- **判定経路**: 窓の app_id の `foot.p<pid>` → その pid の /proc → tty の前面プロセス

見つけ方 3 本の 2 本目。Herdr は起動しない(`session: {herdr: false}`)——それがこの showcase の
主張で、聞く相手がいなくても端末の中は見える。締めの 1 枚は 3 本で同じ文言。

---

## 0. 撮影前の準備

> `./scripts/demo --showcase terminal --record` で生成する。下の表の秒と字幕は生成物と同期して
> ある(`--validate` が見る)。

- 映すアプリ: foot の窓 2 枚。中身は `less` と `vi` の stub(どちらも sheet を開いて見せる)
- 窓の配置は 2 枚が同時に見えるようにずらしてある。pid の違う 2 枚、が画で要る
- **映さない**: 実コード、ホームパスに含まれるユーザー名

## 1. 本編
<!-- variant: main -->

16:9。合計 81 秒。

### §1 端末 1 枚(0:00–0:33)

| 秒 | 画面 | 字幕 |
|---|---|---|
| 0–5 | foot の窓 1 枚。中で less が sheet を開いている | multiplexer を使っていない端末でも |
| 5–11 | 同じ画面 | foot / kitty / Ghostty に対応 |
| 11–19 | `Super+H`。sub-header は `foot · less` | /proc を辿り、tty の前面にいるプロセスを取る |
| 19–26 | 同じ画面 | 端末は foot.p<pid> の app_id で起動している |
| 26–33 | 同じ画面 | 配線は scripts/setup-terminals。既定は dry run |

### §2 端末 2 枚(0:33–1:06)

| 秒 | 画面 | 字幕 |
|---|---|---|
| 33–36 | 2 枚目の foot が開く。中は vi | (字幕なし) |
| 36–43 | 窓は 2 枚、overlay はまだ less の sheet | 窓が 2 枚でも、pid で取り違えない |
| 43–51 | `Super+H` → sheet が `Vi` に差し替わる | 別の窓で押し直せば、その窓のプロセス |
| 51–58 | 同じ画面 | 端末そのものの sheet は要らない |
| 58–66 | vi の窓に `less.yaml` の `match:` が見えている | 端末の sheet を書くなら ^foot$ のまま |

### §3 切り分け(1:06–1:21)

| 秒 | 画面 | 字幕 |
|---|---|---|
| 66–74 | 同じ画面 | wayhint context の chain が空なら端末と見ていない |
| 74–81 | `Super+H` で閉じる | 見つけ方は 3 通り。書き方は 1 つ、match に名前を書くだけ |

## 2. 撮影後のチェック

- [ ] sub-header が `less` → `vi` と変わっている
- [ ] session に herdr / foot-herdr のプロセスが 1 つも無い
- [ ] 画面にユーザー名・実コード・ホームパスが映っていない
- [ ] 字幕帯(y=638 から下)に窓と overlay がかかっていない

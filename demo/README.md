...
設計の理由は `docs/DECISIONS.md` の 0031(生成のしくみ)と 0032(showcase と Herdr)。
...
| grip でのリサイズ | マウス操作 |
| 「エディタで編集」→ gvim | 外部エディタの起動は未検証。YAML の中身は vi の stub で見せる |
...
3. 製品用語は英字のまま: `hint` `sheet` `overlay` `hotkey` `pane` `context`。
   「ヒント」「シート」とは書かない
...
5. 「窓」は字幕では使わない。focus 中の toplevel は「ウィンドウ」、器と中身の対比も
   「ウィンドウ」と「中のプロセス」。「窓」は台本の `画面` 列(内部記述)でだけ使ってよい
6. 親子は**プロセス(context)の関係**として書く。「親 sheet」「子 sheet」「親子 sheet」は使わない
   (実装用語。`docs/DESIGN.md` では `parent_context` の sheet の略として残っている)。
   例: 「親子関係のプロセスは自動で hint を混ぜる」

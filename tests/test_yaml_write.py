...
class WriteDocumentTest(TmpSheetTest):
    ...
    def test_a_symlinked_sheet_stays_a_link_and_its_target_is_written(self) -> None:
        """A sheet kept in a dotfiles repository and linked into hints/ must stay linked."""
        dotfiles = self.dir / "dotfiles"
        dotfiles.mkdir()
        real = dotfiles / "messy.yaml"
        real.write_bytes((DIRTY / "messy.yaml").read_bytes())
        link = self.dir / "linked.yaml"
        link.symlink_to(Path("dotfiles") / "messy.yaml")  # relative, as dotfiles tools make them
        doc = self.doc(link)
        doc["priority"] = 7
        before = os.lstat(link).st_mtime_ns
        os.utime(link, ns=(before - 10**9, before - 10**9), follow_symlinks=False)
        write_document(link, doc)
        self.assertTrue(link.is_symlink())
        self.assertIn("priority: 7", real.read_text())
        self.assertEqual(sorted(p.name for p in dotfiles.iterdir()), ["messy.yaml"])
        # the link is what hints/ watches: touching it is how the overlay hears of the change
        self.assertGreater(os.lstat(link).st_mtime_ns, before - 10**9)

    def test_a_document_past_the_size_limit_is_not_parsed(self) -> None:
        big = self.dir / "big.yaml"
        big.write_text("id: big\n" + "#" * MAX_DOCUMENT_BYTES + "\n")
        data, issues = read_document(big)
...
class CreateSheetTest(TmpSheetTest):
    ...
    def test_a_carriage_return_in_the_context_cannot_add_keys(self) -> None:
        """An app_id or a process name is whatever the program says it is. A bare ``\\r`` is a
        YAML line break that ruamel does not comment out, so it is written out as ``\\r``."""
        injected = "x\rnested: {parent_tags: [a]}"
        path, doc = create_sheet(
            ctx(
                desktop_app=injected,
                parent_context=injected,
                foreground_process=proc(injected, (injected,)),
            ),
            build_hint({"id": "h", "title": "T"}),
            self.config(),
            hints_dir=self.dir,
        ...
        self.assertIn("desktop_app: x\\rnested", text)
        loaded, issues = read_document(path)
        self.assertEqual(issues, [])
        self.assertNotIn("nested", loaded)

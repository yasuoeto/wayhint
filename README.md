...
| コピー | 選択中のヒントをクリップボードへ。`copy` → `command` → `key` の順に、最初にある値 |
...
- 先頭に `#名前` と書くと category で絞る。`Tab` / `Shift+Tab` で category を順に切り替える
  (`#-` は category の無いヒント)。
- `Enter` で選択中のヒント(打つたびに先頭行が選ばれる)をコピーして検索を抜ける。一覧にフォーカスが
  あるときは `c` か `Enter`。抜けると keyboard は元のウィンドウに戻る。
...
| `copy` | コピーする文字列が表示と違うときだけ書く。省略時は `command`、次に `key` |
| `remark` | 選択したときだけ出る補足 |

...
"""The largest sheet or config.yaml read. A hand-written sheet is a few kilobytes; one far past
this is a mistake (or a file from elsewhere), and parsing it would stall the overlay's main
...
def _write_checked(path: Path, doc: object, check: Callable[[object], list[Issue]]) -> None:
    ...
    is touched afterwards: that is the change the overlay sees and reloads on. The new content
    is on disk before it replaces the old (``fsync``), so a crash leaves one or the other, never
    an empty sheet.
    """
    link = path if path.is_symlink() else None
    ...
    doc.yaml_set_start_comment(
        "\n".join(
            [
                ...
                # These come from the window and the process, which any program sets as it
                # likes. A bare ``\r`` ends a YAML line as ``\n`` does, but ruamel only puts
                # ``#`` after ``\n``: left in, it would turn the rest into real keys.
                f"desktop_app: {visible(ctx.desktop_app or '-')}",
                f"parent_context: {visible(ctx.parent_context or '-')}",
                f"foreground_process: {visible(proc.name) if proc else '-'}",
                f"match: {list(regex)}",
            ]
        )
    )

...
- **窓 → プロセスは app_id の接尾辞で解く**: Wayland の protocol も labwc も toplevel の pid を
  ...
  端末と launcher の設定手順は `../docs/TERMINALS.ja.md`。
...
使う人向けの全項目の説明は `../docs/CONFIG.ja.md`。項目を足す・変えるときは両方を直す。
...
使う人向けの全項目の説明は `../docs/SHEET-FORMAT.ja.md`。schema を変えるときは両方を直す。
...
- **他 sheet の hint の混ざり方**(親 sheet と `include`)は `../docs/SHEETS.ja.md` にまとめた。同文書の各節の
...
消えるタイミングは、図付きで `../docs/HOTKEYS.ja.md` にまとめてある。実装は `daemon.py` の `toggle` /
...
引数一覧は `../README.ja.md`「CLI」を参照。`wayhint schema` は PATH 省略時は `editor.schema_path`、
...
scenario の書き方は `../demo/README.ja.md`。
...
- V1 の限界は `PRODUCT.ja.md`「Out of scope」のとおり。

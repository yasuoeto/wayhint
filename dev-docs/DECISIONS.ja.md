...
## 0044 — 文書は英語を本来の名前に、日本語版を `<name>.ja.md` に置く

- **Date**: 2026-09-25
- **Status**: accepted
- **Context**: リポジトリを GitHub で公開する。文書はすべて日本語で、日本語を読まない人は導入も
  使い方も分からなかった。
- **Decision**: README、`docs/`、`dev-docs/`、`demo/README.md`、demo の台本は、`<name>.md` を英語に、
  `<name>.ja.md` を隣の日本語版にする。対になる文書はタイトルの次の行で互いにリンクする。変更は同じ
  commit で両方に入れる。`STATUS.md` と `AGENTS.md` は 1 言語のまま。demo の台本も同じ名前の付け方で、
  それぞれの言語の字幕と突き合わせる。`scripts/setup-terminals` はロケールの言語で出力する
  (wayhint の UI と同じ規則)。
- **Alternatives**: 文書のツリーごとに `ja/` ディレクトリを作る(日本語版へのリンクがすべて動き、
...
- **Decision**:
  ...
  - 1 MiB を超えるシートと `config.yaml` は読まない。正規表現には app_id・引数・コマンドラインの
    先頭 4096 文字までを渡す。
- **Decision(追記、同日の 2 回目のレビュー)**: 書き込みで経由する一時ファイルは `mkstemp` で作る。
  名前を先回りしてリンクを置かれることがなく、最初から `0600` で、中身を書く前に書き込み先の mode に
  する。検証するのはメモリ上の dump 結果で、一時ファイルは読み戻さない。1 MiB の上限はバイトで数える
  (文字数で数えていた)。パーサーが再帰しきれないほど深い文書は `RecursionError` を出さず、間違いと
